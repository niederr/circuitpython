...
static void get_address(bleio_adapter_obj_t *self, ble_gap_addr_t *address) {
    check_nrf_error_with_call_id(1, sd_ble_gap_addr_get(address));
}
...
void common_hal_bleio_adapter_set_enabled(bleio_adapter_obj_t *self, bool enabled) {
    ...
    check_nrf_error_with_call_id(2, err_code);
...
bool common_hal_bleio_adapter_get_enabled(bleio_adapter_obj_t *self) {
    ...
    check_nrf_error_with_call_id(3, sd_softdevice_is_enabled(&is_enabled));
...
mp_obj_t common_hal_bleio_adapter_start_scan(bleio_adapter_obj_t *self, uint8_t *prefixes, size_t prefix_length, bool extended, mp_int_t buffer_size, mp_float_t timeout, mp_float_t interval, mp_float_t window, mp_int_t minimum_rssi, bool active) {
    ...
    if (self->current_advertising_data != NULL && self->current_advertising_data == self->advertising_data) {
        check_nrf_error_with_call_id(4, NRF_ERROR_BUSY);
    }
    ...
    size_t max_packet_size = true ? BLE_GAP_SCAN_BUFFER_EXTENDED_MAX_SUPPORTED : BLE_GAP_SCAN_BUFFER_MAX;
    ...
    check_nrf_error_with_call_id(5, _update_identities(true));
    ...
    if (err_code != NRF_SUCCESS) {
        ...
        self->scan_results = NULL;
        check_nrf_error_with_call_id(6, err_code);
    }
...
mp_obj_t common_hal_bleio_adapter_connect(bleio_adapter_obj_t *self, bleio_address_obj_t *address, mp_float_t timeout) {
    ...
    if (err_code != NRF_SUCCESS) {
        ...
        check_nrf_error_with_call_id(7, err_code);
    }
    ...
    check_nrf_error_with_call_id(8, sd_ble_gattc_exchange_mtu_request(conn_handle, BLE_GATTS_VAR_ATTR_LEN_MAX));
    check_nrf_error_with_call_id(9, sd_ble_gap_data_length_update(conn_handle, NULL, NULL));
...
static bool advertising_on_ble_evt(ble_evt_t *ble_evt, void *self_in) {
    ...
    switch (ble_evt->header.evt_id) {
        case BLE_GAP_EVT_CONNECTED: // Connecting also stops an advertisement.
            ...
            sd_ble_gap_tx_power_set(BLE_GAP_TX_POWER_ROLE_CONN, ble_evt->evt.gap_evt.conn_handle, 3);
            common_hal_bleio_adapter_stop_advertising(self);
            return false;
            break;
...
uint32_t _common_hal_bleio_adapter_start_advertising(bleio_adapter_obj_t *self,
    ...
    err_code = sd_ble_gap_tx_power_set(BLE_GAP_TX_POWER_ROLE_ADV, adv_handle, 3);
    if (err_code != NRF_SUCCESS) {
        return err_code;
    }
...
void common_hal_bleio_adapter_start_advertising(bleio_adapter_obj_t *self, bool connectable,
    ...
    check_nrf_error_with_call_id(10, _common_hal_bleio_adapter_start_advertising(self, connectable, anonymous, timeout, interval,
...
void common_hal_bleio_adapter_stop_advertising(bleio_adapter_obj_t *self) {
    ...
    if ((err_code != NRF_SUCCESS) && (err_code != NRF_ERROR_INVALID_STATE)) {
        check_nrf_error_with_call_id(11, err_code);
    }
}
